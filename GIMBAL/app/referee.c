...
void Judge_Buffer_Receive_Task(uint8_t *frame)
{
 ...
 switch (cmd_id)
    {
			case CONTROLING_COMMAND_ID:
				memcpy(&JudgeReceive.remote_control_t,frame + index,12);
				remote_controller.ms_t=gimbal_time.ms_count;
				remote_controller.s_t	=	gimbal_time.s_count;
			remote_controller.state=1;
			break;
