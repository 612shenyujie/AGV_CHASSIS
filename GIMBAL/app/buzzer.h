...
// （数字越小优先级越高）
typedef uint8_t BUZZER_BEEP_TASK_TO_PRIORITY_LUT_T;
enum {
...
typedef uint16_t BUZZER_DJI_STARTUP_TICK_TO_TONE_LUT_T;
enum {
...
// 规定校准中各音调的时间点
typedef uint16_t BUZZER_CALIBRATING_TICK_TO_TONE_LUT_T;
enum {
...
// 规定校准完成各音调的时间点
typedef uint16_t BUZZER_CALIBRATED_TICK_TO_TONE_LUT_T;
enum {
...
// 规定设备离线各音调的时间点
typedef uint16_t BUZZER_DEVICE_OFFLINE_TICK_TO_TONE_LUT_T;
enum {
...
BUZZER_RETURN_T buzzer_playTone(buzzer_t *buzzer,
...
BUZZER_RETURN_T buzzer_setTask(buzzer_t *buzzer,
                               BUZZER_BEEP_TASK_TO_PRIORITY_LUT_T task);
